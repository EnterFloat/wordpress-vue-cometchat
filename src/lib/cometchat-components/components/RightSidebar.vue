<template>
  <div
    class="ccl-right-panel"
    v-bind:style="isMenu ? 'display: block;' : 'display: none;'"
  >
    <a
      href="javascript:void(0);"
      class="ccl-right back-btn"
      v-on:click="backToCenter()"
      >Back</a
    >
    <div class="ccl-dtls-panel-wrap">
      ...
      <div class="ccl-dtls-panel-body">
        <div class="ccl-dtls-noti-sel-wrap">
          ...
          />
          <label for="notificationSel" class="ccl-semi-bold-text"
            >Notifications</label
          >
        </div>
...
export default {
  ...
  methods: {
    ...
    sidebarlogout() {
      ...
      CometChat.logout()
      .then(() => {
      console.log("Logged out Pushing to AppLoggedOut")
        this.$router.push({
          name: "AppLoggedOut",
        });
      })
...
.ccl-dtls-noti-sel-wrap input[type="checkbox"] + label,
.ccl-more-noti-sel-wrap input[type="checkbox"] + label {
  display: block;
  cursor: pointer;
  background: url(./../assets/images/svg/checkbox-inactive.svg) right center /
    16px no-repeat;
  font-size: 15px;
...
.ccl-dtls-noti-sel-wrap input[type="checkbox"]:checked + label,
.ccl-more-noti-sel-wrap input[type="checkbox"]:checked + label {
  background: url(./../assets/images/svg/checkbox-blue-active.svg) no-repeat
    right center;
...
.ccl-dtls-panel-media-fltr-btn::before {
  content: "";
  position: absolute;
  display: block;
  width: 2px;
...
.ccl-dtls-panel-media-fltrs .ccl-dtls-panel-media-fltr-btn:last-child::before,
.ccl-dtls-panel-media-fltr-btn.active::before {
  display: none;
}
...
.dark-theme .ccl-left-panel-call-fltrs,
.dark-theme .ccl-dtls-panel-media-fltrs {
  background-color: rgba(255, 255, 255, 0.1);
}
.dark-theme .ccl-left-panel-call-fltr-btn.active,
.dark-theme .ccl-dtls-panel-media-fltr-btn.active {
