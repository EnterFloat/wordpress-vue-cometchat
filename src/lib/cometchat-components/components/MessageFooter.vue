  ...
  <div class="cc1-chat-win-inpt-ext-wrap">
    <div class="cc1-chat-win-inpt-int-wrap" v-if="user.uid || group.guid">
      <div class="cc1-chat-win-inpt-attach">
        ...
        <input
          @change="onMediInputChange($event)"
          id="image-input"
          accept="image/*"
          type="file"
          name="name"
      ...
      <!--START-->
      <div
        v-if="filesInput || imageInput || audioInput || videoInput"
        class="cometchat-image-viewer-container"
      >
        <div class="media-wrap-container">
          <img
            v-if="imageInput"
            :src="imageInput"
            class="cometchat-image-viewer-image"
          />
          <audio
            controls
            v-else-if="audioInput"
            class="cometchat-image-viewer-image"
          >
          ...
          <a
            href="javascript:;"
            v-on:click="sendImageMessage($event)"
            class="media-send-button"
            >Send File</a
          >
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      // currentUser: '',
      listItems: [
        { title: "Image", id: "image", icon: "image_icon.svg" },
        { title: "File", id: "file", icon: "attach_file_icon.svg" },
      ],
  ...
  methods: {
    ...
    onchangeDetect(ev) {
      this.inputmessage = ev.target.value.toString();
